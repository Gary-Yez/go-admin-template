<template>
  <el-card class="container" shadow="never">
    <PageHeader title="角色管理" description="按角色分配菜单与接口权限，统一管理访问范围" />
    <div v-if="listError" class="mb-[12px]">
      <el-alert :title="listError" type="error" :closable="false" show-icon />
    </div>
    <ColumnTable @sort-change="handleSortChange" storage-key="core/views/sys_role/index:table-1" v-loading="pageLoading" size="large" :data="tableData">
      <template #toolbar>
      ...
      <el-table-column label="操作" :width="380" fixed="right" align="center">
        <template #default="{ row }">
          <div class="table-btn-group">
            <el-button type="primary" icon="Setting" :disabled="row.is_super_admin" plain size="small" @click="()=>handleEditPermission(row.id)">权限管理</el-button>
            <el-button type="primary" icon="CopyDocument" plain size="small" @click="handleCopy(row)">拷贝</el-button>
            <el-button type="primary" icon="Edit" plain size="small" :disabled="row.is_super_admin" @click="()=>handleAdd(row)">修改</el-button>
            <el-button type="danger" icon="Delete" plain size="small" :disabled="row.is_super_admin" @click="()=>handleDelete([row.id])">删除</el-button>
          </div>
        </template>
      </el-table-column>
    <template #empty><el-empty v-if="!pageLoading" :description="listError ? '加载失败，请刷新重试' : '暂无角色'" :image-size="70" /></template>
    </ColumnTable>
    <div class="table-pagination"><el-pagination v-model:current-page="queryForm.page" v-model:page-size="queryForm.limit" :page-sizes="[10,30,50,100]" :total="total" background layout="total, sizes, prev, pager, next, jumper" /></div>
    <FormDialog description="设置角色名称后，可在权限管理中分配菜单与接口权限。" note-icon="UserFilled" v-model="dialogOpen" v-model:form="submitForm" :title="submitForm.id ? '修改角色' : '新增角色'" :on-confirm="handleSubmit">
      <el-form-item label="角色名称" prop="name" :rules="[{required:true,message:'请输入角色名称'}]">
        <el-input size="large" v-model="submitForm.name" placeholder="请输入角色名称"></el-input>
      </el-form-item>
    </FormDialog>
    <FormDialog description="复制当前菜单和 API 权限，副本与原角色独立维护。" note-icon="CopyDocument" v-model="copyOpen" v-model:form="copyForm" title="拷贝角色" confirm-btn-text="确认拷贝" :on-confirm="handleSubmitCopy">
      <el-form-item label="来源角色"><span>{{ copySourceName }}</span></el-form-item>
      <el-form-item label="新角色名称" prop="name" :rules="[{required:true,whitespace:true,message:'请输入新角色名称'}]">
        <el-input size="large" v-model="copyForm.name" maxlength="100" placeholder="请输入新角色名称" />
      </el-form-item>
    </FormDialog>
    <FormDialog description="分别配置菜单可见范围和接口访问权限。" note-icon="Setting" v-model="permissionOpen" v-model:form="permissionForm" title="权限管理" :on-confirm="handleSubmitPermission">
      <el-tabs type="border-card">
        <el-tab-pane label="菜单权限">
          <p class="text-[12px] text-[var(--el-text-color-secondary)] mb-[12px]">默认菜单用于登录和切换角色后的首页，仅可选择可访问的页面。</p>
          <el-tree ref="menuTreeRef" :data="menus" show-checkbox default-expand-all node-key="id" :props="{label:'name',disabled:(data:any)=>data.key === permissionForm.default_menu}" @check="handleMenuCheck">
            <template #default="{node,data}">
              <div class="role-menu-node">
                <span>{{ data.name }}</span>
                <el-tag v-if="data.key === permissionForm.default_menu" size="small">默认首页</el-tag>
                <el-button v-else-if="data.component && !data.hidden && node.checked" type="primary" link size="small" @click.stop="setDefaultMenu(data)">设为默认</el-button>
              </div>
            </template>
          </el-tree>
        </el-tab-pane>
...
<style scoped>
.role-menu-node { display:flex; align-items:center; gap:10px; width:100%; padding-right:8px; }
.role-menu-node > .el-button { margin-left:auto; }
